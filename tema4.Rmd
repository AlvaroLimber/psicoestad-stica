...
> Nota: Para la interpretación de  la magnitut no tomar en cuenta el signo. El signo sirve para evaluar la dirección de la relación, si es positivo el grupo 1 es mayor al grupo 2 y si es negativo lo opuesto (grupo 2 > grupo 1)
...
La correlación y la regresión lineal permiten cuantificar y modelar la relación entre dos variables cuantitativas.  
En psicología son herramientas fundamentales para analizar vínculos entre constructos tales como ansiedad y rendimiento, estrés y calidad del sueño, o autoeficacia y desempeño académico [@LSJ2025].

---

### Correlación de Pearson (r)

La correlación lineal de Pearson mide la **fuerza** y **dirección** de una asociación lineal entre dos variables:
...
*Ejemplo:*  
Relación entre **ansiedad** y **calidad del sueño**:  
r = −0.42 → asociación negativa moderada.

---

### Correlación no paramétrica: Spearman (ρ)
...
Se basa en **rangos**, no en valores crudos.

---

### Regresión lineal simple

La regresión lineal modela cómo cambia una variable dependiente \(Y\) cuando cambia una variable predictora \(X\).
...
- \(\beta_0\): intercepto  
- \(\beta_1\): efecto esperado en \(Y\) por unidad de cambio en \(X\)  
- \(\epsilon\): error aleatorio

**Interpretación típica:**  
Si β₁ = −0.60 en un modelo donde Y = calidad del sueño y X = estrés,  
entonces **por cada punto adicional de estrés, la calidad del sueño disminuye 0.60 puntos**.

---

### Bondad de ajuste

- **R²**: proporción de varianza de Y explicada por X.  
  - Ej.: R² = 0.28 → el estrés explica el 28% de la variabilidad del sueño.
- **p-value** del β₁ → indica si la relación es estadísticamente significativa.
...
El ANOVA de una vía (one-way ANOVA) evalúa si **tres o más grupos** difieren en su media poblacional [@LSJ2025].

Ejemplos en psicología:
- Diferencias en depresión según tipo de terapia  
- Comparación de memoria según grupo etario  
- Diferencias en motivación por niveles educativos
...
ANOVA compara:
- **Variabilidad entre grupos** (debida al efecto del factor)
- **Variabilidad dentro de los grupos** (debida al error/azar)

El estadístico es:

\[
F = \frac{\text{MS}_{\text{entre}}}{\text{MS}_{\text{dentro}}}
\]

Si F es grande → evidencia contra \(H_0\).

---

### Ejemplo

Tres métodos de relajación se comparan en 60 participantes.  
ANOVA arroja:

- F(2, 57) = 4.91  
- p = 0.011 → significativo  
- η² = 0.15 → efecto moderado

Esto indica que **los métodos difieren en su efecto sobre el nivel de estrés**.

---

### Pruebas post hoc

Si el ANOVA es significativo:

- Tukey HSD  
- Bonferroni  
- Holm

Permiten identificar **qué grupos difieren entre sí**.
...
Estos modelos amplían el ANOVA de una vía para incluir **más de un factor** o **covariables**.

---

### ANOVA factorial

Incluye dos o más factores categóricos.  
Permite estudiar:

1. **Efectos principales** de cada factor  
2. **Interacción** entre factores  

Una interacción implica que **el efecto de un factor depende del nivel del otro**.

---

### Ejemplo factorial 2×2
...
- Efecto principal de terapia: p = .03  
- Efecto principal de género: p = .40  
- Interacción: p = .01  

Interpretación:  
La terapia es más eficaz **solo en mujeres**, indicando interacción significativa.

---

### ANCOVA

El ANCOVA (Análisis de Covarianza) combina:

- factor(es) categórico(s) → como en ANOVA  
- covariable(s) cuantitativa(s)

Objetivo:

- **Controlar una variable cuantitativa que afecta al resultado**,  
- aumentando la precisión del análisis.
...
\[
Y = \mu + \tau_i + \beta X + \epsilon
\]

Donde:

- \(\tau_i\): efecto del grupo  
- \(X\): covariable  
- \(\beta\): efecto ajustador

---

### Ejemplo típico en psicología

Evaluar el efecto de tres terapias en ansiedad, **ajustando por nivel inicial de ansiedad (pretest)**.

Resultados:

- ANOVA simple → no significativo  
- ANCOVA ajustado por pretest → p = .02  

Interpretación:  
Los grupos **sí difieren**, pero solo después de controlar la ansiedad basal.

---

### Supuestos adicionales del ANCOVA

- Linealidad entre Y y covariable  
- Homogeneidad de pendientes (no interacción grupo × covariable)  
- Independencia entre la covariable y el tratamiento

---

### Tamaños del efecto recomendados

- η² (eta cuadrado)  
- η² parcial  
- R² para modelos con covariables  

Valores guía (Cohen 1994):
...
> En conjunto, ANOVA de una vía, ANOVA factorial y ANCOVA permiten estudiar diferencias de medias bajo distintos diseños y estructuras de variables, siguiendo los principios explicados en @LSJ2025 y la lógica clásica de modelos lineales aplicados a psicología y ciencias sociales.
